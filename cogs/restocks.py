...
class Restocks(commands.Cog):
        ...
        for cid in channel_ids:
            ...
            if channel and isinstance(channel, discord.TextChannel):
                if command_name=='empty':
                    ...
                    await thread.send(
                        content=(
                            f"@here 🚨 **Please do NOT ask what is being restocked** until the poster posts what is being restocked, or <t:{unlock_unix}:R>, whichever comes first.\n\n"
    ...
    async def empty(self, interaction: discord.Interaction):
        view=AreaChoiceView(interaction, "empty", self)
        await interaction.response.send_message("Choose a **store**:", view=view, ephemeral=True)
